...
mymaximum bigger initial (a : rest) =
  if bigger a initial
    then mymaximum bigger a rest
...
maybeMap f [] = []
maybeMap f (x : rest) = case f x of
  Just fx -> fx : maybeMap f rest
  Nothing -> maybeMap f rest
