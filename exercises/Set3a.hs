...
firstHalf :: [a] -> [a]
firstHalf a = take (div (length a + 1) 2) a
...
capitalizeFirst (char : rest) = toUpper char : rest
...
joinToLength len list = [first ++ second | first <- list, second <- list, length first + length second == len]
