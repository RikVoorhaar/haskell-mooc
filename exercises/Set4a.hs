...
longest :: (Ord a) => [[a]] -> [a]
longest [x] = x
longest (x : xs) = foldr compareLists x xs
  where
    compareLists xs ys = case compare (length xs) (length ys) of
      GT -> xs
      LT -> ys
      EQ -> if head xs <= head ys then xs else ys
...
-- incrementKey key [] = []
-- incrementKey key ((k, v) : kvs) = (if key == k then (k, v + 1) else (k, v)) : incrementKey key kvs
  ...
  case (Map.lookup from bank, Map.lookup to bank) of
    (Just fromBalance, Just toBalance)
      | amount > 0 && fromBalance >= amount ->
          Map.insert to (toBalance + amount) $ Map.insert from (fromBalance - amount) bank
    _ -> bank
...
maxIndex arr = fst $ foldl1 check (Data.Array.assocs arr)
  where
    check :: (Ix i, Ord a) => (i, a) -> (i, a) -> (i, a)
    check (i, a) (j, b)
      | a > b = (i, a)
      | otherwise = (j, b)
